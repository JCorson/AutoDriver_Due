...
//  the dSPIN chip.
void AutoDriver_Due::setParam(byte param, unsigned long value)
{
  param |= SET_PARAM;
  SPIXfer((byte)param);
...
//  the dSPIN chip.
unsigned long AutoDriver_Due::getParam(byte param)
{
...
void AutoDriver_Due::run(byte dir, float stepsPerSec)
{
  SPIXfer(RUN | dir);
...
void AutoDriver_Due::stepClock(byte dir)
{
...
void AutoDriver_Due::move(byte dir, unsigned long numSteps)
{
  SPIXfer(MOVE | dir);
...
void AutoDriver_Due::goTo(long pos)
{
  SPIXfer(GOTO);
...
void AutoDriver_Due::goToDir(byte dir, long pos)
{
...
void AutoDriver_Due::goUntil(byte action, byte dir, float stepsPerSec)
{
...
//  for act.
void AutoDriver_Due::releaseSw(byte action, byte dir)
{
...
//  ABS_POS values.
void AutoDriver_Due::setMark(long newMark)
{
  setParam(MARK, newMark);
}

void AutoDriver_Due::setPos(long newPos)
{
