...
// AutoDriverSupport_Due.cpp - Contains utility functions for converting real-world
//  units (eg, steps/s) to values usable by the dsPIN controller. These are all
//  private members of class AutoDriver_Due.
...
unsigned long AutoDriver_Due::accCalc(float stepsPerSecPerSec)
{
...
// This is a 12-bit value, so we need to make sure the value is at or below 0xFFF.
unsigned long AutoDriver_Due::decCalc(float stepsPerSecPerSec)
{
  float temp = stepsPerSecPerSec * 0.137438;
  if( (unsigned long) long(temp) > 0x00000FFF) return 0x00000FFF;
  else return (unsigned long) long(temp);
...
unsigned long AutoDriver_Due::maxSpdCalc(float stepsPerSec)
{
...
// This is a 12-bit value, so we need to make sure the value is at or below 0xFFF.
unsigned long AutoDriver_Due::minSpdCalc(float stepsPerSec)
{
...
unsigned long AutoDriver_Due::FSCalc(float stepsPerSec)
{
...
unsigned long AutoDriver_Due::intSpdCalc(float stepsPerSec)
{
  float temp = stepsPerSec * 4.1943;
...
unsigned long AutoDriver_Due::spdCalc(float stepsPerSec)
{
...
unsigned long AutoDriver_Due::paramHandler(byte param, unsigned long value)
{
...
unsigned long AutoDriver_Due::xferParam(unsigned long value, byte bitLen)
{
...
//  is AWESOME, and I've personally discovered it at least twice.
 byte AutoDriver_Due::SPIXfer(byte data)
{
  byte rxData;
  digitalWrite(_CSPin, LOW);
  rxData = SPI.transfer(data);
//  SPDR = data; // SPDR(SPI Data Register)
//  while (!(SPSR&(1<<SPIF))); // wait for SPI to finish operation; SPSR(SPI Status Register)
//  rxData = SPSR;
//  rxData = SPDR;
